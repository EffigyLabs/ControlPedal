...
ResponsiveAnalogRead analogPos4Pin(knobPin, true); // Knob doesn't need snap adjust
...
void setup() {
  ...
  delay(1600); // time for emitters and sensors to level
    ...
    if (
      ...
      for (short slot = 0; slot < 3; slot++) {
        ...
        if ( (livePreset.mode[modeIndex].pos[pos].slot[slot].MidiCmd == 0) || // slot is empty
             ...
             // eventually move the reasons to be silent during noteon here as well?
           ) {
...
//                                       0x2000 * minpct, 0x3FFF * maxpct);
             ...
             case MIDI_NOTEON: {
                 ...
                 if (Inputs[pos][VALUE_COL] > peaks[pos][slot][MAX_VAL_COL]) { // we're attacking so don't send any output yet, midval remains zero
                      ...
                      short xxmidval = constrain(map(peaks[pos][slot][MAX_VAL_COL], Inputs[pos][LL_COL], Inputs[pos][UL_COL], 0, 127), 0, 127); // the value is the velocity    
                 ...
                 if(peaks[pos][slot][ISATTACKING_COL]) { // were going up
                    if(Inputs[pos][VALUE_COL] < Inputs[pos][LL_COL]) { // but at open gate
                        midval = constrain(map(peaks[pos][slot][MAX_VAL_COL], Inputs[pos][LL_COL], Inputs[pos][UL_COL], 0, 127), 0, 127); // the value is the velocity    
...
//                midval = constrain(map(Inputs[pos][VALUE_COL], Inputs[pos][LL_COL], Inputs[pos][UL_COL], 0 * minpct, 127 * maxpct), 0, 127);
          ...
          // the curve type and direction have been applied already//
...
// indicate to debug port.  This output will work on either the Serial monitor or plotter in the arduino IDE
        ...
        } // else the min/max and latching is ok
...
//        EEPROM.put(0,systemBlock);
    ...
    case 14: { // change boot mode
        ...
        if (bm != systemBlock.bootmode) {
          ...
          EEPROM.put(0, systemBlock); // must test this ********** tag *****
        }
      }
      full_message_received = true;
      break;
